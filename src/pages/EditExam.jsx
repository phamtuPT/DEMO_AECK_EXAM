...
import { getExams, updateExam } from "../store/Exam/thunk/examThunk";
...
const EditExam = () => {
  ...
  const loading = useSelector((state) => state.examManagementReducer.loading.update);
  ...
  useEffect(() => {
    if (exams.length > 0) {
      ...
      if (foundExam) {
        setExam(foundExam);
        setSelectedQuestions(foundExam.questionIds || []);
        setExamSettings(foundExam.settings || examSettings);
        ...
        form.setFieldsValue({
          ...
          difficulty: foundExam.difficulty,
  ...
  const onFinish = async (values) => {
    try {
      ...
      const result = await dispatch(updateExam(exam.id, examData));
      ...
      message.error("Có lỗi xảy ra khi cập nhật đề thi");
    }
  };
